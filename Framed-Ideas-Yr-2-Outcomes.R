...
#Only include ideas that have been coded, are from year 2, and are framing or neutral
Yr2_Data <- subset(Data, GroupID == 4
                        & PRConsensusCode != " "
                        & PRConsensusCode != 0)

Idea_Val <- Yr2_Data$PRConsensusCode
...
Interventions <- c("Neutral", "Adaptive", "Innovative")
...
for(i in 1:length(Contexts)){
  for(j in 1:length(Interventions)){
    ideas <- which(Yr2_Data$ContextShortName == Contexts[i] & Yr2_Data$FramingDesc == Interventions[j])
...
con <- 1
for (k in seq(from=2, to=13, by=3)){
  AN_colors <- c("red", "orange")
  IN_colors <- c("red", "blue")
  barplot(Prop_Mat[c(k,k+1),], ylim=c(0,0.8), beside=TRUE, col=AN_colors, main=c(Contexts[con], "Year 2 Neutral and Adaptive"))
  legend("topright", c("Neutral", "Adaptive"), fill=AN_colors)
  
  barplot(Prop_Mat[c(k,k+2),], ylim=c(0,0.8), beside=TRUE, col=IN_colors, main=c(Contexts[con], "Year 2 Neutral and Innovative"))
...
rownames(Prop_Mat) <- c("Blank", "Neutral Snow", "Adaptive Snow", "Innovative Snow", "Neutral Lids", "Adaptive Lids", "Innovative Lids",
                        "Neutral Belongings", "Adaptive Belongings", "Innovative Belongings", "Neutral Rainwater", "Adaptive Rainwater", "Innovative Rainwater")
colnames(Prop_Mat) <- c("1", "2", "3", "4")

colors <- c("darkslateblue", "darkorchid3", "hotpink3", "indianred2")
con <- 1
for (j in seq(from=2, to=13, by=3)){
  barplot(t(Prop_Mat[c(j,j+1,j+2),]), col=colors, main=c(Contexts[con], "Year 2"))
