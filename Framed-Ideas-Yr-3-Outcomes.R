...
Yr3_Data <- subset(Data, GroupID == 5
                        & PRConsensusCode != " "
                        & PRConsensusCode != 0)

Idea_Val <- Yr3_Data$PRConsensusCode
...
for(i in 1:length(Contexts)){
  for(j in 1:length(Interventions)){
    ideas <- which(Yr3_Data$ContextShortName == Contexts[i] & Yr3_Data$FramingDesc == Interventions[j])
...
for (k in seq(from=2, to=7, by=3)){
  ...
  barplot(Prop_Mat[c(k,k+1),], ylim=c(0,0.8), beside=TRUE, col=AN_colors, main=c(Contexts[con], "Year 3 Neutral and Adaptive"))
  legend("topright", c("Neutral", "Adaptive"), fill=AN_colors)
  
  barplot(Prop_Mat[c(k,k+2),], ylim=c(0,0.8), beside=TRUE, col=IN_colors, main=c(Contexts[con], "Year 3 Neutral and Innovative"))
...
for (j in seq(from=2, to=7, by=3)){
  barplot(t(Prop_Mat[c(j,j+1,j+2),]), col=colors, main=c(Contexts[con], "Year 3"))
